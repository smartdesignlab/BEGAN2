...
class Trainer(object):
    def __init__(self, config, data_loader):
        ...
        self.n_buffer = config.n_buffer

        _, height, width, self.channel = \
                get_conv_shape(self.data_loader)
    ...
    def train(self):
        ...
        cost_file = open(self.model_dir+ "/cost.txt", 'w', self.n_buffer)
        ...
        for step in trange(self.start_step, self.max_step):
            ...
            if step % self.log_step == 0:
                ...
                k_t = result['k_t']
                cost_file.write("Epoch: ["+str(step)+"/"+str(self.max_step)+"] d_loss: "+str(d_loss)+", g_loss:"+ str(g_loss)+" measure: "+str(measure)+", k_t: "+ str(k_t)+ "\n")
    ...
    def build_model(self):
        ...
        G, self.G_var = Generator(
                self.z, self.conv_hidden_num, self.channel,
                self.repeat_num, reuse=False)

        d_out, self.D_z, self.D_var = Discriminator(
                tf.concat([G, x], 0), self.channel, self.z_num, self.repeat_num,
                self.conv_hidden_num)
        AE_G, AE_x = tf.split(d_out, 2)

        self.G = denorm_img(G)
        self.AE_G, self.AE_x = denorm_img(AE_G), denorm_img(AE_x)
    ...
    def build_test_model(self):
        ...
        G_z_r, _ = Generator(
                self.z_r, self.conv_hidden_num, self.channel, self.repeat_num, reuse=True)
